      ...
      <div className="px-lg py-base flex flex-col gap-lg">
        <Card className="bg-canvas-soft dark:bg-surface-dark-elevated">
          ...
          <div className="flex items-baseline justify-between">
            <p className="text-display-sm font-display text-ink dark:text-on-dark">{formatBRL(balances.totalBRL)}</p>
          ...
          <div className="grid grid-cols-2 gap-sm">
            {pessoas.map((p) => (
              <Card key={p}>
                ...
                <p className="text-title-md font-sans text-ink dark:text-on-dark">{formatBRL(balances.pago[p] ?? 0)}</p>
            ...
            <Card>
              <div className="flex flex-col divide-y divide-hairline dark:divide-hairline-dark">
                {gastosOrdenados.map((g) => (
                  <GastoRow key={g.id} gasto={g} onEdit={() => openEdit(g)} />
                ))}
              </div>
            </Card>
          )}
